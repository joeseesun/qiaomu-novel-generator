...
description: |
  Generate original, complete, highly gripping Chinese short fiction from a story theme, character setup, synopsis, trope, or draft excerpt. Use when the user asks for 小说, 短篇小说, 故事, 网文样章, 开篇钩子, 悬念改写, 人物欲望强化, 情节升级, 续写, or wants a blend of excellent writers' general narrative techniques without copying protected text or imitating a living author's distinctive style.
---

# Qiaomu Novel Generator

把一句主题、人物设定、梗概或已有片段，生成原创、完整、强钩子、高张力、可继续扩写的中文短篇小说。
...
- The user wants a complete Chinese short story, not only an outline.
...
- Treat named writers as craft signals, not surface-style targets. Convert them into general techniques such as suspense, restraint, moral pressure, voice economy, scene rhythm, reversal, and emotional payoffs.
...
1. Intent Hook: identify the input type, target genre, reader promise, and likely quality rubric before drafting.
2. Story Engine Hook: extract:
   - protagonist desire
   - visible obstacle
   - hidden pressure
   ...
   - reader promise
   - final aftertaste
3. Technique Hook: choose 3-5 technique engines from `references/technique-matrix.md`. Use a mix, not a stack of author imitations.
4. Plan Hook: build a compact story plan:
...
5. Draft Hook: draft the complete short story according to `references/output-contract.md`.
6. Quality Hook: self-check against `references/quality-checklist.md` and the relevant rubric in `references/genre-quality-rubric.md`. Revise before returning if the story fails on hook, desire, escalation, dialogue, image, reversal/suspense, or ending aftertaste.
7. Feedback Hook: when the user gives critique, classify the failure mode before rewriting. Do not only patch the current paragraph.
8. Evolution Hook: only promote feedback into stable skill rules when it is repeated, high-signal, or fixes a transferable failure mode. See `references/evolution-loop.md`.
9. If writing a sample, deliverable, or file artifact, run `python3 scripts/validate_skill.py` from the skill directory before calling it done. When a draft file exists, optionally run `python3 scripts/evaluate_story.py <draft-file>` for a lightweight metrics report.

## Output Defaults

For a normal user request, output:
